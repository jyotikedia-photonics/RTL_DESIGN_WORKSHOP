...
Now let us simulate this incomplete if statement and see what wave forms do we get. As we can see, When i 0 is 1 the output why is following I one input but When our Io  is going low , the output y is latching onto some previous value, it's either 1 or 0 permanently. there is no change absolutely . thus clearly the latching action is taking place. Lets us see in synthesis. 
